...
>
> **문서 버전**: Final v2.8.1 (2026-07-06) — v2.8 통합본의 정합성 검수판
...
프레임워크는 (i) 다포맷 텍스트 추출 (HTML/HWP/PDF/DOCX/RTF/XLSX), (ii) **19종 한국어 smell taxonomy** 정의 (Paska 매핑 6종 + 한국 특화 4종 + ISO 29148/INCOSE/EARS 5종 + CMMI/NCS 4종), (iii) 5 detector 앙상블 탐지, (iv) 6 표준 리포트 정량 평가의 4단계로 구성된다. LLM/휴리스틱 기반 자동 교정 단계는 예비 설계만 제시하고 실증은 후속 연구로 남긴다. 검증은 평가 대상을 **두 카테고리로 분리**하여 수행했다 — **[A] RFP 평가(발주 단계)**: 공공금융 RFP 56건 비정제(D1), 정형 RFP 30건(D2), 공공 다도메인 RFP 사례 14건 (의료·법무·식약·U-City·교육·DB구축 등) 4,075 요구사항(D5); **[B] 요구사항정의서 평가(수행 단계)**: 실기업 4모듈 257건 익명화(D4), **실기업 9프로젝트 정의서 14,278건 익명화(D6)**; 여기에 PURE 영문 SRS 79건(D3)을 기준선으로 둔다. 정밀화 과정에서 자동 likely-FP를 **26% → 3.9%(-22pp)** 감소시켰으며, 4 방식 baseline 비교에서 **Rule-only가 Macro-F1 0.278로 최선**(LLM-assisted는 dry-run이라 Rule과 동률)임을 정량 확인했다. D5 실증에서 **87.5% smell 검출**, 상위 검출 패턴은 **S18 추적ID부재 (69.7%)·S2 불완전 (26.2%)·S5 주체누락 (16.6%)**로 한국어 SI 작성 관행이 도메인 무관하게 우세함을 입증했으며, 같은 D5 내부에서 **양식 정형성 효과(-82pp)가 도메인 변경 효과(+22pp)의 3.7배**임을 발견했다. 표준 정렬 진화는 **v1.0 ~20% → v2.0 28% → v2.7 55% → v2.8 ~70%**, CMMI 9 원칙 충족도는 **5/9 → 8/9**, NCS 5 제약 카테고리 분류 보조는 **0/5 → 5/5**로 확장되었다. 본 도구는 오픈소스(<https://github.com/24nga/KoFinRe>)로 공개되며, 실기업 데이터 처리를 위한 5단계 윤리·법적 워크플로우와 함께 [`experiments/rfp_2013_sample/`](https://github.com/24nga/KoFinRe/tree/main/experiments/rfp_2013_sample)에 재현 가능한 다도메인 샘플 실험을 제공한다.

**Keywords:** Korean Requirements Engineering, Multi-domain Requirements Quality, Requirement Smell, Paska, IEEE 830, ISO 29148, INCOSE, EARS, CMMI REQM/RD, NCS Constraint Categories, NLP Ensemble, Anonymization
...
> LLM 기반 교정의 의미 보존 검증(구 RQ5)은 본 논문 범위에서 제외하고 후속 연구 질문으로 이관한다(§8.2).

### 1.4 기여 (7종)

1. **19종 한국어 smell taxonomy**: Paska 매핑 6종(S1·S3·S5·S6·S8·S9) + 한국 특화 4종(S2·S4·S7·S10) + ISO/INCOSE/EARS 5종(S11~S15) + CMMI/NCS 4종(S16~S19) — 도메인 무관 일반 루브릭
...
4. **2 카테고리 6 데이터셋 검증**: **[A] RFP 평가** — 공공금융(D1·D2) + 공공 다도메인 RFP 사례 D5 (의료·법무·식약·U-City·교육·DB구축 등) / **[B] 요구사항정의서 평가** — 실기업 익명화 D4 + **실기업 9프로젝트 14,278건 D6** / 영문 기준선(D3)
5. **다도메인 일반화 입증**: D5에서 한국어 SI 작성 관행이 도메인 무관하게 우세함을 정량 확인 (87.5% smell, Top 3: S18·S2·S5) + **양식 정형성 > 도메인** 발견 (효과비 3.7×)
...
```
RFP 문서 → ① 추출 → ② Smell 정의(19종) → ③ 앙상블 탐지 → ④ 정량 평가
```

> 본 논문의 프레임워크는 위 4단계로 구성된다. 검출 이후의 자동 교정(LLM/휴리스틱)은 도구에 예비 구현되어 있으나(§3.6), 실증 평가는 후속 연구로 이관한다.
...
| DOCX | python-docx |
| RTF | striprtf |
| XLSX | openpyxl (정형 요구사항 명세서 — ID·분류·명칭·내용 컬럼 직접 파싱) |
...
| S10 | 검증불가 | Verifiability | 한국 특화 + IEEE 830 | v2.0 |
...
| **Correction** (후속 연구용 예약) | Smell Reduction Rate, Quality Score Gain, Semantic Preservation, Over-correction, Atomicity·Testability Improvement |

리포트 6종: `extraction_report.md`, `smell_report.md`, `evaluation_report.md`, `correction_report.md`, `dataset_card.md`, `run_log.json`.

### 3.6 교정 확장 (예비 설계 — 본 논문 범위 외, 후속 연구)

> 검출된 smell을 자동 교정하는 확장 단계의 예비 설계다. 도구에 이중 모드(LLM/휴리스틱)로 구현되어 있으나, 의미 보존·과잉 보완에 대한 실증 평가는 수행하지 않았으므로 본 논문의 기여 범위에 포함하지 않고 후속 연구(§8.2)로 이관한다.
...
### 4.1 데이터셋 — 평가 대상 2 카테고리 + 기준선 (MECE 분리, 다도메인)

평가 대상 문서를 라이프사이클 단계에 따라 **두 카테고리로 분리**한다. 두 문서 유형은 작성 주체·양식·품질 이슈 프로파일이 본질적으로 다르므로(§6.4), 결과 보고와 해석도 카테고리별로 수행한다.

**카테고리 A — RFP 평가 (발주 단계, 발주기관 작성)**
...
**카테고리 B — 요구사항정의서 평가 (수행 단계, SI 수행사 작성)**

| 코드 | 데이터 | 도메인 | 크기 | 출처 | 위치 |
|---|---|---|---:|---|---|
| **D4** | 실기업 4 모듈 (FA·BG·CI·CM) | 캐피탈 | 257 req | 한 국내 캐피탈사 | **🔒 로컬 only** |
| **D6** | **실기업 9 프로젝트 정의서** (2011~2020) | 금융 (은행·캐피탈·보증·공제) | **116 파일 / 14,278 req** | 익명화 (P-A~P-H) | **통계만 공개** (`experiments/d6_enterprise_specs/`) |
...
> \* D1 문장 수는 파이프라인 버전에 따라 상이: v1 추출 3,210 문장 (§6.1의 필터 통과율 기준), v2 재추출 4,917 문장. 본 표는 v2 기준.
>
...
>
> **카테고리 구분 근거** — RFP는 발주기관이 사업 범위를 서술하는 비정형·서술형 문서(노이즈 필터링이 선행 과제), 정의서는 수행사가 1행 1요구사항 양식으로 작성하는 정형 산출물(전수 행 단위 평가 가능). 추출 파이프라인(Stage 1)은 카테고리 A에만 적용되며, 카테고리 B는 양식 파싱으로 직접 입력된다.
...
| LLM-assisted | Regex + LLM OR (본 실험에선 dry-run — 실 호출 시 Rule과 분화 가능) |
...
| D2 | 30 req (정제 CSV) | 30 (100%) | 100% | CSV 직접 — 추출 단계 불요 |
...
| **전체 smell 비율** | 46.6% | 65.3% | 56.7% | **79.4%** (v2.3)† | **87.5%** |

> n/a = 해당 시점(v2.0 이전 또는 v2.7 이전)에 미평가. † D4는 정밀화 버전에 따라 93.4%(v2.1.1) → 77.0%(v2.2) → 79.4%(v2.3); 본 표는 최신 v2.3 기준.

### 5.2.1 D5 다도메인 검증 — 핵심 결과

**상위 5 검출 (v2.7/v2.8 신규 패턴 3종이 Top 5에 진입)**
...
> ‡ LLM-assisted는 본 실험에서 dry-run(API 미호출)이므로 Rule-only와 동일 결과. 실 API 호출 시 분화 가능하며 이는 v2.9 후속 과제.

### 5.8 휴리스틱 교정 예비 데모 (범위 외 — 후속 연구 예고)

> 아래 데모는 §3.6 예비 설계의 동작 예시일 뿐, 본 논문의 평가 대상이 아니다. 정량 실증(의미 보존·과잉 보완)은 후속 연구에서 수행한다.
...
### 5.9 단위 테스트 결과
- 전체 스위트: **60/61 통과** (잔여 1건은 약어 화이트리스트 edge case — v2.1부터 알려진 이슈)
- v2.7 신규 smell 테스트 (`tests/test_new_smells.py`): 16/16 통과
...
### 6.1 RQ1 답 (추출 기준)
의무 종결어 7종(`~여야 한다 / 함 / 됨 / 할 수 있어야 / 반드시 / 필수 / ~토록 한다`) + 6 카테고리 제외 룰 + 사이트 공통 텍스트 자동 검출(5+ 사업 일치)로 정밀 필터 가능. D1 비정제 v1 추출분에서 3,210 → 75건(2.3%) 통과, 통과한 75건의 65.3%가 진짜 smell 검출 — 영문 Paska(46.6%)보다 높은 통과율 정확성. D5에서는 동일 필터가 HWP 13건 10,899 문장 → 3,409건(31.3%)을 통과시켜, 요구사항 밀도가 높은 발주 문서에 대한 스케일 확장을 입증했다.
...
### 6.3 RQ3 답 (앙상블 효과) ⭐
**한국어 요구사항 도메인에선 Rule-only가 최선**. 4 방식 비교에서 Macro-F1 0.278(Rule) vs 0.060(NLP) vs 0.244(Ensemble) — LLM-assisted는 dry-run이라 Rule과 동률이며, 실 호출 비교는 후속 과제. 정형 표현 명확성으로 휴리스틱이 효율적이고, NLP·Ensemble은 과검출 경향. **v2.8 신규 S16~S19는 모두 Rule-based 정밀 패턴이라 Rule-only 우위 가설을 강화**한다. D5 다도메인 실증에서 동일 RegexDetector가 14건·4,075 요구사항을 단일 패스로 평가 가능했으며 도메인별 별도 튜닝 불요.

### 6.4 RQ4 답 (빈번 오류 — 카테고리별 프로파일 + 다도메인 일반화 + 표준 커버리지)

**카테고리 A — RFP 평가 (발주 단계)**: 서술형 문서 특성상 주체 생략·모호 표현이 우세하다.

| 우세 패턴 | D1 금융 비정제 | D2 금융 정형 | **D5 다도메인 공공** |
|---|---:|---:|---:|
| 주체누락 (S5) | 32.0% | 26.7% | **16.6%** |
| 불완전 (S2) | — | — | **26.2%** |
| 약한의무 (S4) | 0% | 30.0% | 0.1% |
| 추적ID부재 (S18) | — | — | **69.7%** |

**카테고리 B — 요구사항정의서 평가 (수행 단계)**: 1행 1요구사항 양식이 원자성(S1)은 구조적으로 확보하지만, 불완전 서술·주체 생략이 압도적이다.

| 우세 패턴 | D4 캐피탈 4모듈 (257) | **D6 9프로젝트 (14,278)** |
|---|---:|---:|
| 불완전 (S2) | 91% | **74.4%** |
| 주체누락 (S5) | 81% | **59.0%** |
| 미정의약어 (S7) | 11% | **30.4%** |
...
→ **카테고리별 프로파일이 뚜렷이 다르다**: RFP는 추적ID부재(S18)·주체모호 중심, 정의서는 불완전(S2)+주체누락(S5) 중심. 그러나 두 카테고리 모두 영문 SRS의 Passive voice(D3 27.5%) 우세와 명확히 구분되며, 한국어 SOV 어순·주어 생략 허용·평서형 종결이 문서 유형·도메인을 가로질러 일관되게 관찰된다. 특히 D6은 D4의 결론(S2+S5 지배, S1 우수)을 **55배 규모·9개 기관·10년치**에서 재현하여 정의서 카테고리 결론의 외적 타당성을 확보한다.
...
### 6.5 (범위 외 이관) 교정 관련 논의
자동 교정(구 RQ5)은 본 논문 범위에서 제외하였다. 도구에는 LLM 어댑터(6 원칙 prompt + dry-run fallback)와 휴리스틱 마커 교정(v2.8: S16~S19용 4종 마커)이 예비 구현되어 있으며(§3.6, §5.8 데모), 실제 LLM 호출 기반 의미 보존·과잉 보완 정량 평가는 후속 연구(§8.2)에서 다룬다.

### 6.6 종합 시사점
- **도구**: 한국어 요구사항 도메인은 휴리스틱이 효율적, 표준 정렬로 학술·실무 신뢰성 확보
- **다도메인 일반화**: D1~D6 6 데이터셋(금융·캐피탈·의료·법무·교육·U-City·DB구축 등)에서 한국어 SI 작성 관행의 우세 패턴이 일관 — **도메인 특화가 아닌 일반 루브릭으로 활용 가능**
- **카테고리별 프로파일 분리**: RFP(발주)는 S18 추적ID부재·주체모호 중심, 정의서(수행)는 S2 불완전+S5 주체누락 중심 — **평가·개선 권고는 문서 유형별로 달라야 함** (RFP: ID·출처 체계 / 정의서: 완전한 문장 서술·주체 명시 교육)
- **양식 정형성 우선**: 같은 D5 내부에서 정형 XLSX(17.3%) vs 평면 HWP(99~100%) 차이가 도메인 간 차이보다 큼 (효과비 3.7×) — **한국어 요구공학 품질 개선의 ROI 1순위는 양식 정형화**
...
- v2.5 baseline 비교에서 gold label로 사용한 R1_sim 50건이 휴리스틱 기반이라 **Rule-only에 유리한 편향** 가능. LLM-assisted가 dry-run이라 Rule과 동률인 점도 같은 원인.
- v2.8 detector 테스트는 합성 예문 18종 — 실제 데이터셋에서의 P/R 미측정
- **S14 수혜자불명이 D5에서 0% 검출** — SYSTEM_ACTOR(시스템·서버 등)가 persona로 과대 인정되는 detector 보정 이슈 의심. persona 사전과 시스템 행위자 사전의 분리 재조정 필요 (v2.9)
- 잔여 단위 테스트 1건 실패 (약어 화이트리스트 edge case, v2.1부터 지속)

### 7.2 External Validity (외적 타당성)
- D1~D4가 금융/캐피탈 도메인 편중 — **D5 추가로 의료·법무·교육·DB구축·U-City 등 9+ 도메인 일반화는 확보**, 다만 국방·통신·자율주행 등은 여전히 미검증
- D4는 한 국내 캐피탈사의 단일 시점 산출물 — 산업 평균 대표성 한계. **D6(9개 기관·2011~2020·14,278건) 추가로 정의서 카테고리의 기관·시점 대표성은 크게 보완**되었으나, D6 역시 금융권 편중이며 제조·공공행정 정의서는 미검증
- 카테고리 간 비교(§6.4)는 문서 유형과 함께 모수 정의(RFP: 필터 통과 후보 / 정의서: 전수 행)가 달라지므로 방향성 해석에 한정
...
- S18 추적성은 ID 패턴 5종으로 한정 — 사내 고유 형식 누락 가능. D5의 S18 69.7%도 "HWP 본문 서술형에 ID가 없는 것이 관행"임을 감안하면 심각도 조정 여지 있음 (양식 유형별 가중치가 후속 과제)
- D1(비정제)과 D4·D5(정형/발주)의 "smell 비율"은 모수 정의가 달라 (필터 통과 요구사항 후보 기준) 데이터셋 간 절대 비교에는 주의 필요
...
### 8.1 결론
본 연구는 한국어 SW/IT 요구사항 품질 자동 평가의 4 공백(데이터셋·도구·taxonomy·표준 정렬)을 해결하기 위한 **도메인 무관 한국어 요구사항 평가 루브릭 KoFinRe-QA Framework**를 제안하고, **2 평가 카테고리(RFP / 요구사항정의서)·6 데이터셋·6대 표준**으로 검증했다. 주요 결과:

1. **19종 한국어 smell taxonomy** 정의 (Paska 매핑 6종 + 한국 특화 4종 + ISO/INCOSE/EARS 5종 + CMMI/NCS 4종)
...
4b. **D6 정의서 대규모 검증** — 실기업 9프로젝트 14,278건 87.0% smell (S2 74.4%·S5 59.0%), D4 결론의 55배 규모 재현 — **RFP/정의서 카테고리별 프로파일 분리** 확립
5. **양식 정형성 > 도메인 발견** — 양식 효과(-82pp)가 도메인 효과(+22pp)의 3.7배
6. **6대 표준 정렬** — v1.0 ~20% → v2.8 ~70%, CMMI 5/9 → 8/9, NCS 0/5 → 5/5
7. **RQ3 답**: 한국어 도메인에선 Rule-only 최선 (Macro-F1 0.278) — v2.8 신규 패턴이 가설 강화
8. **RQ4 답**: 한국어 요구사항 품질 이슈는 정제 수준이나 도메인이 아니라 **언어·문화적 작성 관행 + 양식 정형성**
9. **5단계 윤리·법적 워크플로우** 표준 제안 + **재현 가능한 실험 폴더** (`experiments/rfp_2013_sample/`) 공개 — 후속 연구의 비교 기준선

### 8.2 Future Work
| 방향 | 작업 | 목표 버전 |
|---|---|---|
| 평가자 데이터셋 | 실제 사람 평가자 200건 + Cohen's kappa + 진짜 P/R/F1 | v3.0 |
| **자동 교정 단계 실증 (본 논문 범위 외 이관)** | LLM 실제 호출 + 의미 보존·과잉 보완 정량 평가 + 휴리스틱 마커 교정 유효성 검증 + LLM-assisted baseline 재비교 — 구 RQ5를 독립 후속 연구로 수행 | v2.9 |
| 일관성 smell (S20) | 의미 임베딩 기반 요구사항 간 충돌 탐지 → CMMI 9/9 완전 충족 | v2.9 |
| S14 detector 보정 | persona 사전 / 시스템 행위자 사전 분리 (D5 0% 이슈) | v2.9 |
| EARS 권장 도구 (S21) | Ubiquitous/Event/State/Optional/Unwanted 패턴 권장 | v3.0 |
| 양식 정형성 지수 | form_formality_score 정의 (S18·S15·S5 가중 합) — 양식 유형별 smell 심각도 조정 | v3.0 |
| 도메인 확장 | 잔여 도메인 (국방·통신·제조·자율주행) RFP 검증 + 최신 (2020년대) 공공 RFP 시점 비교 | v3.1 |
...
│   ├── correction_heuristic.py  # 휴리스틱 교정 (S16~S19 마커 포함)
│   ├── korean_patterns.py  # 한국어 패턴 + CMMI/NCS 사전 (v2.8)
│   ├── smell_taxonomy.yaml # 19종 정식 정의 (source_standard·introduced_in)
│   ├── ensemble.py
│   ├── metrics.py
│   ├── reporting.py
│   └── validation.py
├── docs/                   # 활성 문서 (v2.8)
│   ├── PAPER_FINAL.md             # 본 논문 (v2.8.1 최종본)
│   ├── CMMI_NCS_COMPARISON.md     # CMMI/NCS 분석
│   ├── DOMAIN_COMPARISON.md       # 금융 vs 공공 다도메인 비교
│   ├── JOURNEY.md                 # 전체 여정 (13단계)
│   └── UPDATE.MD                  # 변경 이력
├── old/                    # 시점별 historical 보관
...
├── experiments/            # 재현 가능한 샘플 실험
│   └── rfp_2013_sample/           # D5: stage1 추출 + stage3 평가 산출물
├── scripts/                # CLI + PDF 번들 + D5 파이프라인
...
│   └── test_metrics.py
├── web/index.html          # 브라우저 평가기 (SheetJS, 19종 동기화)
└── legacy/                 # v1 보관
```
...
- D5는 공개 발주 사례이므로 익명화 불요 — 단, 원본 HWP/XLSX 바이너리는 저장소 제외 (추출 CSV·통계만 공개)

### C. 재현성
- 저장소: <https://github.com/24nga/KoFinRe>
- 의존성: `pip install -r requirements.txt` (kiwipiepy 포함; HWP 추출은 Windows + 한컴오피스 + pywin32)
- 검증: `python -m unittest discover tests` + `python examples/basic_usage.py`
- **D5 재현**: [`experiments/rfp_2013_sample/README.md`](https://github.com/24nga/KoFinRe/tree/main/experiments/rfp_2013_sample)의 4단계 레시피 (CP949 ZIP 해제 → HWP 추출 → XLSX 추출 → v2.8 평가)
- 보조 문서:
  ...
  - [`DOMAIN_COMPARISON.md`](./DOMAIN_COMPARISON.md) — 금융 vs 공공 다도메인 비교 9 섹션
  - [`../old/STANDARDS_COMPARISON.md`](../old/STANDARDS_COMPARISON.md) — v2.7 시점 IEEE/ISO/INCOSE/EARS 갭 (historical)
...
| §4 Setup | 데이터 5종·지표·baseline·윤리 | ✓ 독립 |
| §5 Results | 정량 결과 + D5 다도메인 + 도메인 비교 + 표준 커버리지 | ✓ 독립 |
| §6 Discussion | RQ 답 5종 + 종합 시사점 | ✓ 독립 |
| §7 Validity | 위협 4종 (detector 보정 이슈 포함 정직 보고) | ✓ 독립 |
| §8 Conclusion | 종합 9결과·향후 9방향 | ✓ 독립 |
| References | 15 인용 | — |
| Appendix | 보조 자료 5종 | — |

전체 연구 내용 포괄 — 누락 없음. v2.8의 4종 smell + 6대 표준 정렬 + D5 다도메인 검증이 §2.2 / §3.3 / §4.1 / §5.2~5.5 / §6.2~6.4 / §7.2 / §8.1에 일관성 있게 통합되었음.
