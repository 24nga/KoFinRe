...
### 4.1 데이터셋 — 평가 대상 2 카테고리

평가 대상 문서를 라이프사이클 단계에 따라 **RFP 평가(발주 단계, 발주기관 작성, 비정형 서술)**와 **요구사항정의서 평가(수행 단계, 수행사 작성, 정형 양식)**의 두 카테고리로 분리한다. 두 유형은 작성 주체·양식·모수 정의가 다르므로 결과 보고와 해석도 분리한다.
...
| **D-PRIV-L** | 국내 금융권 실기업 정의서 9프로젝트 (2011~2020, 116파일) | **14,278 req** | 보조 검증 — D-PRIV 결론의 55배 규모 재현 | 🔒 비공개* (통계만 공개: `experiments/d6_enterprise_specs/`) |
...
> \* **D-PRIV·D-PRIV-L 비공개 사유**: 계약상 비밀유지 의무 및 영업비밀 보호 대상 문서로, 8종 식별자(회사·인명·사번·연락처·이메일·URL·일자·협력사) 정규식 익명화를 적용한 후에도 도메인 용어 조합에 의한 재식별 위험이 존재한다. 이에 원문·익명화본 모두 저장소 외부에 격리하고 논문에는 통계량만 제시한다. 연구 재현성은 공개 데이터셋 D-MAIN으로 보장한다.
...
### 5.4 요구사항정의서 카테고리 검증 (D-PRIV / D-PRIV-L)

민간 금융권 정형 양식 257건(D-PRIV)에서 완전성 계열(불완전 91%, 주체누락 81%)이 우세하였고, 이 패턴은 **9개 기관·2011~2020년·14,278건(D-PRIV-L)에서도 재현**되었다(전체 87.0% 검출, 불완전 74.4%·주체누락 59.0%·미정의약어 30.4%, 복합의무 1.7%; 비공개 사유로 통계량만 제시). 정형 양식임에도 셀 내부 서술이 불완전한 사례가 다수로, **양식 정형화는 필요조건이지 충분조건이 아님**을 시사한다.

카테고리 간 프로파일도 뚜렷이 다르다: RFP(D-MAIN)는 추적ID부재(69.7%) 중심, 정의서(D-PRIV·D-PRIV-L)는 불완전+주체누락 중심 — 개선 권고는 문서 유형별로 달라야 한다(RFP: ID·출처 체계 / 정의서: 완전한 문장 서술·주체 명시).
...
2. D-MAIN이 2013년 발주 사례 — 최신 작성 관행과의 시차 존재 (요구사항 상세화 제도 도입기 사례 분석으로 시점 한정). 정의서 카테고리는 D-PRIV-L(2011~2020)로 시점 폭을 보완했으나 금융권 편중
